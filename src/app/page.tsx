...
export default function Home() {
    ...
    // ── Threat Ranking & Nearby ───────────────────────────────────────────────
    const [threatRanking, setThreatRanking] = useState<{ name: string; score: number; status: "Safe" | "Warning" | "Critical" }[]>([])
    const [nearbyObjects, setNearbyObjects] = useState<{ name: string; distKm: number }[]>([])
    const [selectedSatVelocity, setSelectedSatVelocity] = useState<number | null>(null)
    ...
    const runCollisionDetection = useCallback(() => {
        ...
        // ── Threat Ranking: top 5 riskiest satellites ──────────────────────────
        const satScoreMap = new Map<string, number>()
        sats.forEach(s => {
            // Base score from risk level
            const base = s.riskLevel === "high" ? 70 + Math.random() * 30
                : s.riskLevel === "medium" ? 35 + Math.random() * 35
                    : s.riskLevel === "low" ? 10 + Math.random() * 25
                        : Math.random() * 10
            satScoreMap.set(s.name, parseFloat(base.toFixed(1)))
        })
        // Boost from conjunction involvement
        found.forEach(c => {
            const boost = c.risk === "high" ? 25 : c.risk === "medium" ? 12 : 5
            satScoreMap.set(c.satA, Math.min(99.9, (satScoreMap.get(c.satA) ?? 0) + boost))
            satScoreMap.set(c.satB, Math.min(99.9, (satScoreMap.get(c.satB) ?? 0) + boost))
        })
        const ranking = Array.from(satScoreMap.entries())
            .sort((a, b) => b[1] - a[1])
            .slice(0, 5)
            .map(([name, score]) => ({
                name,
                score: parseFloat(score.toFixed(1)),
                status: score >= 65 ? "Critical" as const : score >= 30 ? "Warning" as const : "Safe" as const,
            }))
        setThreatRanking(ranking)
    }, [])
    ...
    const flyToSat = useCallback((sat: SatObject) => {
        ...
        setActiveTab("info")
        setSearchOpen(false)
        const vel = parseFloat((Math.sqrt(398600 / (6371 + sat.alt))).toFixed(2))
        setSelectedSatVelocity(vel)
        const others = satellitesRef.current
            .filter(s => !s.destroyed && s.name !== sat.name)
            .map(s => ({ name: s.name, distKm: parseFloat((sat.position3D.distanceTo(s.position3D) * 500).toFixed(1)) }))
            .sort((a, b) => a.distKm - b.distKm)
            .slice(0, 3)
        setNearbyObjects(others)
    }, [])
    ...
    useEffect(() => {
        ...
        renderer.domElement.addEventListener("click", (event) => {
            ...
            if (intersects.length > 0) {
                ...
                // Compute velocity and nearby objects on click
                const vel = parseFloat((Math.sqrt(398600 / (6371 + sat.alt))).toFixed(2))
                setSelectedSatVelocity(vel)
                const others = satellitesRef.current
                    .filter(s => !s.destroyed && s.name !== sat.name)
    ...
    return (
      ...
      `}</style>
                ...
                {activeLeftTab === "status" && (<>
                    ...
                    <div style={{ marginTop: 6, marginBottom: 12 }}>
                        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 8, fontSize: 11 }}>
                            ...
                            <span style={{
                                color: speedUI >= 1000 ? "#ff4422" : speedUI >= 100 ? "#ffaa00" : "#00ccff",
                                fontFamily: "'Orbitron', sans-serif", fontWeight: 900, fontSize: 13,
                                textShadow: speedUI >= 1000 ? "0 0 10px rgba(255,68,34,0.6)" : speedUI >= 100 ? "0 0 10px rgba(255,170,0,0.5)" : "0 0 8px rgba(0,200,255,0.5)",
                                transition: "all 0.3s ease",
                            }}>{speedUI}×</span>
                        </div>
                        <div style={{ display: "flex", gap: 5 }}>
                            {[1, 10, 100, 1000].map(preset => (
                                <button key={preset} onClick={() => changeSpeed(preset)} style={{
                                    flex: 1, padding: "7px 0",
                                    background: speedUI === preset
                                        ? preset >= 1000 ? "rgba(255,68,34,0.25)" : preset >= 100 ? "rgba(255,170,0,0.2)" : "rgba(0,200,255,0.2)"
                                        : "rgba(0,10,30,0.6)",
                                    border: `1px solid ${speedUI === preset
                                        ? preset >= 1000 ? "rgba(255,68,34,0.7)" : preset >= 100 ? "rgba(255,170,0,0.6)" : "rgba(0,200,255,0.6)"
                                        : "rgba(0,180,255,0.15)"}`,
                                    borderRadius: 3,
                                    color: speedUI === preset
                                        ? preset >= 1000 ? "#ff4422" : preset >= 100 ? "#ffaa00" : "#00ffcc"
                                        : "rgba(150,200,255,0.4)",
                                    fontFamily: "'Orbitron', sans-serif", fontSize: 10, fontWeight: 700,
                                    cursor: "pointer", letterSpacing: "0.5px",
                                    boxShadow: speedUI === preset
                                        ? preset >= 1000 ? "0 0 10px rgba(255,68,34,0.3)" : preset >= 100 ? "0 0 10px rgba(255,170,0,0.25)" : "0 0 10px rgba(0,200,255,0.25)"
                                        : "none",
                        ...
                        <div style={{ fontSize: 9, color: "rgba(150,200,255,0.3)", marginTop: 5, textAlign: "center", fontFamily: "'Share Tech Mono', monospace", letterSpacing: "0.5px" }}>
                            {speedUI === 1 ? "REAL-TIME" : speedUI === 10 ? "FAST FORWARD" : speedUI === 100 ? "HYPERSPEED" : "⚡ WARP SPEED"}
                        </div>
                    </div>
                ...
                {activeTab === "info" && (
                    <>
                        {selectedSat ? (
                            <>
                                {/* Satellite header */}
                                <div style={{ marginBottom: 14, padding: "10px 12px", background: "rgba(0,200,255,0.04)", border: "1px solid rgba(0,200,255,0.12)", borderRadius: 4 }}>
                                    <div style={{ fontFamily: "'Orbitron', sans-serif", fontSize: 13, color: "#00ccff", fontWeight: 700, letterSpacing: "1px", marginBottom: 3 }}>
                                        {selectedSat.flag} {selectedSat.name.slice(0, 24)}
                                    </div>
                                    <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
                                        <span style={{ fontSize: 9, fontFamily: "'Orbitron', sans-serif", letterSpacing: "2px", padding: "2px 8px", borderRadius: 2, background: selectedSat.orbitType === "LEO" ? "rgba(0,255,255,0.1)" : selectedSat.orbitType === "MEO" ? "rgba(255,255,0,0.1)" : "rgba(255,68,0,0.1)", color: selectedSat.orbitType === "LEO" ? "#00ffff" : selectedSat.orbitType === "MEO" ? "#ffff44" : "#ff4400", border: `1px solid ${selectedSat.orbitType === "LEO" ? "rgba(0,255,255,0.3)" : selectedSat.orbitType === "MEO" ? "rgba(255,255,0,0.3)" : "rgba(255,68,0,0.3)"}` }}>{selectedSat.orbitType}</span>
                                        <span style={{ fontSize: 10, color: "rgba(150,200,255,0.4)" }}>ORBIT</span>
                                    </div>
                                </div>
                                ...
                                <div className="stat-row">
                                    ...
                                    <span className="stat-value" style={{ color: "#00ffcc" }}>
                                        {selectedSatVelocity != null ? `${selectedSatVelocity} km/s` : "—"}
                                    </span>
                                </div>
                                ...
                                {/* Risk Score */}
                                {(() => {
                                    const riskEntry = threatRanking.find(t => t.name === selectedSat.name)
                                    const score = riskEntry?.score ?? (selectedSat.riskLevel === "high" ? 65 + Math.floor(Math.random() * 30) : selectedSat.riskLevel === "medium" ? 30 + Math.floor(Math.random() * 35) : selectedSat.riskLevel === "low" ? 10 + Math.floor(Math.random() * 20) : Math.floor(Math.random() * 10))
                                    const scoreColor = score >= 65 ? "#ff2200" : score >= 30 ? "#ffaa00" : "#00ff88"
                                    const scoreBg = score >= 65 ? "rgba(255,34,0,0.12)" : score >= 30 ? "rgba(255,170,0,0.1)" : "rgba(0,255,136,0.08)"
                                    const scoreBorder = score >= 65 ? "rgba(255,34,0,0.35)" : score >= 30 ? "rgba(255,170,0,0.3)" : "rgba(0,255,136,0.2)"
                                    return (
                                        <div style={{ margin: "12px 0", padding: "10px 12px", background: scoreBg, border: `1px solid ${scoreBorder}`, borderRadius: 4 }}>
                                            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 6 }}>
                                                <span style={{ fontFamily: "'Orbitron', sans-serif", fontSize: 8, letterSpacing: "1.5px", color: "rgba(150,200,255,0.6)" }}>RISK SCORE</span>
                                                <span style={{ fontFamily: "'Orbitron', sans-serif", fontWeight: 900, fontSize: 16, color: scoreColor, textShadow: `0 0 12px ${scoreColor}88` }}>{score.toFixed(1)}%</span>
                                            </div>
                                            <div style={{ height: 5, background: "rgba(255,255,255,0.07)", borderRadius: 3, overflow: "hidden" }}>
                                                <div style={{ height: "100%", width: `${score}%`, background: `linear-gradient(90deg, ${score >= 65 ? "#ff4400, #ff0000" : score >= 30 ? "#ff8800, #ffcc00" : "#00cc66, #00ff88"})`, borderRadius: 3, transition: "width 0.6s cubic-bezier(0.4,0,0.2,1)", boxShadow: `0 0 8px ${scoreColor}66` }} />
                                            </div>
                                            <div style={{ marginTop: 5, display: "flex", justifyContent: "space-between", fontSize: 9, color: "rgba(150,200,255,0.4)", fontFamily: "'Share Tech Mono', monospace" }}>
                                                <span>0 — NOMINAL</span>
                                                <span style={{ color: scoreColor, fontFamily: "'Orbitron', sans-serif", fontWeight: 700, fontSize: 8, letterSpacing: "1px" }}>{score >= 65 ? "CRITICAL" : score >= 30 ? "WARNING" : "NOMINAL"}</span>
                                ...
                                <div style={{ marginTop: 4, padding: "10px 12px", background: "rgba(255,60,0,0.06)", border: "1px solid rgba(255,100,0,0.2)", borderRadius: 3 }}>
                                    <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 6 }}>
                                ...
                                {nearbyObjects.length > 0 && (
                                    <div style={{ marginTop: 10, padding: "10px 12px", background: "rgba(0,150,255,0.04)", border: "1px solid rgba(0,150,255,0.18)", borderRadius: 4 }}>
                                        <div style={{ fontFamily: "'Orbitron', sans-serif", fontSize: 8, color: "rgba(100,180,255,0.6)", letterSpacing: "2px", marginBottom: 8 }}>CLOSEST OBJECTS</div>
                                        {nearbyObjects.map((obj, i) => (
                                            <div key={i} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "5px 0", borderBottom: i < nearbyObjects.length - 1 ? "1px solid rgba(0,180,255,0.07)" : "none" }}>
                                                <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
                                                    <span style={{ color: i === 0 ? "#ffaa00" : "rgba(150,200,255,0.3)", fontSize: 9 }}>#{i + 1}</span>
                                                    <span style={{ fontSize: 10, color: "#c8e8ff" }}>{getCountryFlag(obj.name)} {obj.name.slice(0, 16)}</span>
                                                </div>
                                                <span style={{ fontSize: 10, color: obj.distKm < 100 ? "#ff6644" : obj.distKm < 300 ? "#ffcc44" : "rgba(150,200,255,0.5)", fontWeight: "bold" }}>{obj.distKm} km</span>
                                            </div>
                                        ))}
                                    </div>
                                )}
            ...
            <div className="panel" style={{
                position: "fixed", bottom: 24, right: 24, width: 260, padding: "14px 16px", zIndex: 10,
            }}>
                <div style={{ fontFamily: "'Orbitron', sans-serif", fontSize: 9, color: "#ff4422", letterSpacing: "2px", marginBottom: 10, display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                    <span>⚠ THREAT RANKING</span>
                    <span style={{ color: "rgba(150,200,255,0.35)", fontSize: 8 }}>TOP 5</span>
                </div>

                {threatRanking.length === 0 ? (
                    <div style={{ textAlign: "center", padding: "12px 0", color: "rgba(150,200,255,0.3)", fontSize: 10 }}>Computing...</div>
                ) : (
                    <>
                        {/* Header row */}
                        <div style={{ display: "flex", justifyContent: "space-between", fontSize: 8, color: "rgba(150,200,255,0.35)", fontFamily: "'Share Tech Mono', monospace", letterSpacing: "1px", marginBottom: 5, padding: "0 2px" }}>
                            <span style={{ flex: 1 }}>SAT NAME</span>
                            <span style={{ width: 40, textAlign: "right" }}>SCORE</span>
                            <span style={{ width: 52, textAlign: "right" }}>STATUS</span>
                        </div>
                        {threatRanking.map((entry, i) => {
                            const statusColor = entry.status === "Critical" ? "#ff2200" : entry.status === "Warning" ? "#ffaa00" : "#00ff88"
                            const statusBg = entry.status === "Critical" ? "rgba(255,34,0,0.15)" : entry.status === "Warning" ? "rgba(255,170,0,0.12)" : "rgba(0,255,136,0.08)"
                            return (
                                <div key={i} style={{
                                    display: "flex", justifyContent: "space-between", alignItems: "center",
                                    padding: "5px 6px", marginBottom: 3,
                                    background: i === 0 ? `${statusBg}` : "rgba(0,10,30,0.4)",
                                    border: `1px solid ${i === 0 ? statusColor + "44" : "rgba(0,180,255,0.08)"}`,
                                    ...
                                    <div style={{ flex: 1, overflow: "hidden" }}>
                                        <div style={{ display: "flex", alignItems: "center", gap: 4 }}>
                                            <span style={{ fontSize: 8, color: "rgba(150,200,255,0.35)", fontFamily: "'Orbitron', sans-serif" }}>#{i + 1}</span>
                                            <span style={{ fontSize: 10, color: "#c8e8ff", whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>{entry.name.slice(0, 16)}</span>
                                        </div>
                                        {/* Mini score bar */}
                                        <div style={{ marginTop: 3, height: 2, background: "rgba(255,255,255,0.06)", borderRadius: 1 }}>
                                            <div style={{ height: "100%", width: `${entry.score}%`, background: statusColor, borderRadius: 1, transition: "width 0.5s ease" }} />
                                        </div>
                                    </div>
                                    <span style={{ width: 40, textAlign: "right", fontSize: 11, fontWeight: "bold", color: statusColor, fontFamily: "'Share Tech Mono', monospace" }}>{entry.score}%</span>
                                    <span style={{
                                        width: 52, textAlign: "right", fontSize: 8, fontFamily: "'Orbitron', sans-serif",
                                        letterSpacing: "0.5px", padding: "2px 4px", borderRadius: 2,
                                        background: statusBg, color: statusColor,
                                        animation: entry.status === "Critical" ? "pulse-high 1.5s infinite" : "none",
            ...
            <div style={{ position: "fixed", bottom: 310, right: 24, zIndex: 10, display: "flex", flexDirection: "column", gap: 6 }}>
            ...
            <button onClick={toggleTrails} style={{
                position: "fixed", bottom: 270, right: 24, zIndex: 10,
